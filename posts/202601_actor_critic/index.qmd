...
Briefly, the reward prediction error is a concept in reinforcement that gradually tracked more attention of neuroscientists. At each time step $t$, the animal resides in a state $s_t$. Depending on the state, the animal receive a reward with its value given by $r_t$.  Hence, each possible state is associated with a so-called value $V(s_t)$, which is defined as:
...
Although RPE signal is most commonly reported to be present in the dopaminergic system, it does not prevent us from using this framework to understand how reward-association learning happens in the hippocampus.
...
::: {.cell}
![Graph for transition of chief baker](./images/family_graph.png){width=60% fig-align="center"}
:::
...
::: {.cell}
![The probability of whether cookies are baked depending on each family member being the chief baker](./images/cookie_baking.png){width=60% fig-align="center"}
:::
...
For more advanced knowledge about the hidden Markov model, one could check out this [tutorial](https://engrxiv.org/preprint/view/765/) by Benyamin Ghojogh etc.